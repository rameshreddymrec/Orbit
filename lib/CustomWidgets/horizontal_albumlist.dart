...
class HorizontalAlbumsList extends StatelessWidget {
  ...
  String getSubTitle(Map item) {
    ...
    if (type == 'charts') {
      ...
      final sub = formatString(item['subtitle']?.toString()).trim();
      return (sub == '0 Songs' || sub == '0 Song') ? 'Playlist' : sub;
    } else if (type == 'song') {
      return formatString(item['artist']?.toString());
    } else {
      if (item['subtitle'] != null) {
        final sub = formatString(item['subtitle']?.toString()).trim();
        if (sub != '0 Songs' && sub != '0 Song') return sub;
      }
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          // if (index < 3) {
          //   print('HorizontalAlbumsList render: title="${item["title"]}", subtitle="$subTitle", type="${item["type"]}", count="${item["count"]}", song_count="${item["more_info"]?["song_count"]}", song_pids="${item["more_info"]?["song_pids"]}"');
          // }
              ...
              showDialog(
                ...
                builder: (context) {
                  return AlertDialog(
                    ...
                    content: imageCard(
                      borderRadius:
                          item['type'] == 'radio_station' ? 1000.0 : 20.0,
