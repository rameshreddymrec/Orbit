...
import 'package:orbit/CustomWidgets/animated_text.dart';
...
import 'package:orbit/CustomWidgets/equalizer.dart';
import 'package:orbit/CustomWidgets/like_button.dart';
import 'package:orbit/CustomWidgets/seek_bar.dart';
import 'package:orbit/CustomWidgets/snackbar.dart';
import 'package:orbit/CustomWidgets/textinput_dialog.dart';
import 'package:orbit/Helpers/audio_service_helper.dart';
import 'package:orbit/Helpers/config.dart';
import 'package:orbit/Helpers/dominant_color.dart';
...
class _PlayScreenState extends State<PlayScreen> {
  ...
  void updateBackgroundColors(String? uri, List<Color?> value) {
    if (uri != null) {
      _colorCache[uri] = value;
    }
    ...
    return Dismissible(
      ...
      child: StreamBuilder<MediaItem?>(
        ...
        builder: (context, snapshot) {
          ...
          if (artUri != null && artUri != '') {
            if (_colorCache.containsKey(artUri)) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                gradientColor.value = _colorCache[artUri];
              });
            } else {
              artUri.startsWith('file')
                  ? getColors(
                    ...
                    ).then((value) => updateBackgroundColors(artUri, value))
                  : getColors(
...
class _PlayingAnimationState extends State<PlayingAnimation>
  ...
  final List<double> _heights = [0.2, 0.8, 0.4, 0.7];
        ...
        return Row(
          ...
          children: List.generate(4, (index) {
            return Container(
              ...
              width: 3,
              height: 15 * (_heights[index] + (1 - _heights[index]) * _controller.value),
              decoration: BoxDecoration(
                color: Theme.of(context).colorScheme.secondary,
      ...
      Lyrics.getOffLyrics(
        ...
        if (value == '' && widget.getLyricsOnline) {
          Lyrics.getLyrics(
            ...
            saavnHas: widget.mediaItem.extras?['has_lyrics'].toString() == 'true' ||
                widget.mediaItem.extras?['has_lyrics'].toString() == '1',
            title: widget.mediaItem.title,
            artist: widget.mediaItem.artist.toString(),
            album: widget.mediaItem.album?.toString(),
            language: widget.mediaItem.extras?['language']?.toString(),
            duration: widget.mediaItem.duration,
          ).then((Map value) {
            lyrics['lyrics'] = value['lyrics'];
            lyrics['type'] = value['type'];
            if (lyrics['type'] == 'text') {
              lyrics['lyrics'] = Lyrics.autoSync(
          ...
          SlidingUpPanel(
            ...
            header: GestureDetector(
              ...
              onVerticalDragUpdate: (DragUpdateDetails details) {
                // Only allow dragging up (negative dy) to open, disable dragging down (positive dy) to close
                if (details.delta.dy < 0.0) {
                     panelController.open();
                }
              },
