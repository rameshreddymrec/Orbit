...
import 'package:orbit/Helpers/playlist.dart';
import 'package:orbit/Screens/Player/audioplayer.dart';
import 'package:orbit/Services/isolate_service.dart';
...
class AudioPlayerHandlerImpl extends BaseAudioHandler
  ...
  Future<void> _init() async {
    // Configure logging in the isolate
    Logger.root.level = Level.ALL;
    Logger.root.onRecord.listen((record) {
      print('${record.level.name}: ${record.time}: ${record.message}');
    });
    Logger.root.info('DEBUG: Starting AudioPlayerHandlerImpl initialization');
    ...
    try {
      ...
      await Hive.openBox('stats'); // Needed for adding to stats
      downloadsBox = Hive.box('downloads');
      Logger.root.info('DEBUG: Hive initialized successfully in isolate');
      _readyCompleter.complete();
    } catch (e) {
      Logger.root.severe('DEBUG: Failed to initialize Hive in AudioService isolate', e);
    }
    ...
    Hive.box('settings').watch().listen((event) {
      if (event.key == 'streamingQuality') {
        preferredMobileQuality = event.value.toString();
      }
      if (event.key == 'streamingWifiQuality') {
        preferredWifiQuality = event.value.toString();
      }
      if (event.key == 'resetOnSkip') {
        resetOnSkip = event.value as bool;
      }
      if (event.key == 'cacheSong') {
        cacheSong = event.value as bool;
      }
      if (event.key == 'autoplay') {
        recommend = event.value as bool;
      }
      if (event.key == 'loadStart') {
        loadStart = event.value as bool;
      }
      preferredQuality = connectionType == 'wifi'
          ? preferredWifiQuality
          : preferredMobileQuality;
  ...
  Future<AudioSource?> _itemToSource(MediaItem mediaItem) async {
    ...
    try {
      Logger.root.info('Creating audiosource for ${mediaItem.title}');
      if (mediaItem.artUri.toString().startsWith('file:')) {
        ...
        if (downloadsBox != null &&
          ...
          audioSource = AudioSource.uri(
            ...
            String audioUrl = mediaItem.extras!['url'].toString();
            if (audioUrl.contains('_96.')) {
  ...
  Future<List<AudioSource>> _itemsToSources(List<MediaItem> mediaItems) async {
    ...
    final futures = mediaItems.map(_itemToSource);
  ...
  Future<void> startService() async {
    ...
    if (withPipeline && !kIsWeb && Platform.isAndroid) {
  ...
  Future<void> updateQueue(List<MediaItem> newQueue) async {
    _playlist = ConcatenatingAudioSource(children: await _itemsToSources(newQueue));
    try {
      await _player!.setAudioSource(_playlist, initialIndex: _pendingIndex ?? 0);
      _pendingIndex = null;
    } catch (e) {
      Logger.root.severe('Error in setAudioSource', e);
    }
  ...
  Future customAction(String name, [Map<String, dynamic>? extras]) {
    ...
    if (name == 'setInitialIndex') {
      _pendingIndex = extras?['index'] as int?;
    }
