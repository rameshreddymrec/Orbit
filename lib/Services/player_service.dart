...
class PlayerInvoke {
      ...
      queue.addAll(
        response.map(
          (song) => MediaItem(
            ...
            duration: Duration(
              seconds: MediaItemConverter.parseDuration(song['duration']),
            ),
  ...
  static Future<void> setValues(
    ...
    // URL fetching is now handled by audio_service on demand
  ...
  static Future<void> updateNplay(List<MediaItem> queue, int index) async {
    await audioHandler.customAction('setInitialIndex', {'index': index});
